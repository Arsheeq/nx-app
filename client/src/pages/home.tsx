...
export default function Home() {
  ...
  const [formAccountName, setFormAccountName] = useState(accountName || (cloudProvider === 'AWS' ? 'My AWS Account' : 'My Azure Account'));
  ...
  const [formAzureClientSecret, setFormAzureClientSecret] = useState(azureClientSecret);
  const [formAzureTenantId, setFormAzureTenantId] = useState(azureTenantId);
  const [formAzureSubscriptionId, setFormAzureSubscriptionId] = useState(azureSubscriptionId);

  // Form state for billing period
  const [formBillingMonth, setFormBillingMonth] = useState(billingMonth.toString());
  const [formBillingYear, setFormBillingYear] = useState(billingYear.toString());

  // Mutation for validating AWS credentials
  const validateAwsMutation = useMutation({
    mutationFn: async (credentials: any) => {
      const response = await apiRequest('POST', '/api/aws/validate', credentials);
  ...
  const validateAzureMutation = useMutation({
    mutationFn: async (credentials: any) => {
      const response = await apiRequest('POST', '/api/azure/validate', credentials);
      return response.json();
    },
    onSuccess: (data) => {
      setCloudAccountId(data.accountId);
  ...
  const handleNext = () => {
    const currentStepData = steps.find((step, index) => index + 1 === currentStep);
    ...
    if (currentStepData?.id === 'credentials') {
      ...
      nextStep();
    } else if (currentStepData?.id === 'resources' || currentStepData?.id === 'frequency' || currentStepData?.id === 'report-type' || currentStepData?.id === 'cloud-provider') {
      ...
      const formattedResources = selectedResources.map(resourceId => {
        ...
        const resource = useStore.getState().resources.find(r => r.resourceId === resourceId);
        if (resource) {
        ...
        return (
          <section>
            <div className="text-center mb-8">
              ...
              <p className="text-gray-500 mt-2">
                Provide your {cloudProvider} access credentials to scan for resources
              </p>
            </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input
                    id="account-name"
                    type="text"
                    placeholder={cloudProvider === 'AWS' ? "My AWS Account" : "My Azure Account"}
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormAzureClientSecret(e.target.value)}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormAzureSubscriptionId(e.target.value)}
                      />
                    </div>
                  </>
                )}
                ...
                <div className="mt-2 text-sm text-gray-500">
                  <p>Your credentials are only used to scan resources and are never stored.</p>
                </div>
              </div>
            </Card>
          </section>
              ...
              >
                <div className="bg-white border rounded-lg p-6 hover:shadow-md cursor-pointer transition">
                  <div className="flex items-center mb-4">
                    <RadioGroupItem value="daily" id="daily" className="h-4 w-4" />
                    <Label htmlFor="daily" className="ml-2 text-lg font-medium">Daily Report</Label>
                  </div>
                  <div className="ml-6">
                    <p className="text-gray-600 mb-3">Collect resource metrics for the last 24 hours.</p>
                ...
                <div className="bg-white border rounded-lg p-6 hover:shadow-md cursor-pointer transition">
                  <div className="flex items-center mb-4">
                    ...
                    <Label htmlFor="weekly" className="ml-2 text-lg font-medium">Weekly Report</Label>
                  </div>
                  <div className="ml-6">
                    <p className="text-gray-600 mb-3">Collect resource metrics for the last 7 days.</p>
                    <ul className="text-sm text-gray-700 space-y-2">
                      <li className="flex items-start">
                        <CheckCircle className="h-5 w-5 text-[#3DB3E3] flex-shrink-0 mr-1.5" />
            ...
            <Card className="p-6 max-w-2xl mx-auto">
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="flex justify-between py-2 border-b">
                  ...
                  <span>{reportType === 'utilization' ? 'Utilization Report' : 'Billing Report'}</span>
                </div>
                ...
                {reportType === 'utilization' ? (
                  <div className="flex justify-between py-2 border-b">
                    ...
                    <span>{frequency === 'daily' ? 'Daily (24 hours)' : 'Weekly (7 days)'}</span>
                  </div>
                ) : (
                  <div className="flex justify-between py-2 border-b">
                    <span className="font-medium">Billing Period:</span>
                    <span>{getMonthName(billingMonth)} {billingYear}</span>
                  </div>
                )}
                          ...
                          {selectedResources.map(resourceId => {
                            ...
                            return (
                              <li key={resourceId} className="text-sm">
                                {resourceId} ({resource?.name || 'Unknown'}) - {resource?.region || 'Unknown'}
                              </li>
                            );
                          })}
                        </ul>
                  ...
                  <div>
                    {reportType === 'utilization' ? (
                      <p className="text-sm text-blue-800">
                        This report will include CPU, memory, and disk utilization metrics for the selected resources. 
                        Generating the report may take a few moments depending on the number of resources selected.
                      </p>
                    ) : (
                      <p className="text-sm text-blue-800">
                        This report will include detailed billing information for {getMonthName(billingMonth)} {billingYear}.
              ...
              <div className="flex justify-between mt-10">
                ...
                {(steps[currentStep - 1]?.id !== 'credentials' && 
                  ...
                  <Button 
                    ...
                    disabled={
                      submitting || 
                      (steps[currentStep - 1]?.id === 'resources' && selectedResources.length === 0)
                    }
                  >
                    Next
                  </Button>
                ...
                {steps[currentStep - 1]?.id === 'credentials' && !reportId && (
                  <Button 
                    ...
                    disabled={submitting || (
                      ...
                      !formAzureClientId || !formAzureClientSecret || !formAzureTenantId || !formAzureSubscriptionId
                    )}
                  >
                    {submitting ? (
                      <>
